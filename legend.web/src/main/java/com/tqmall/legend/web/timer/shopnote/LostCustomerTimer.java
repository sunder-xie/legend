 ...
 */
@Component
public class LostCustomerTimer extends AbstractTimer {
    @Resource
    ...
    private static final String DEFAULT_KEY = "SHOP_NOTE_LOST_CUSTOMER";

    public LostCustomerTimer() {
        super(LostCustomerTimer.class, "LostCustomerJob", 5000, 5);
    }
            ...
            if (result == 1) {//多机部署只有一台设备会在运行Job
                ...
                for (final Shop shop : shops) {
                    executor.execute(new Runnable() {
                        @Override
                        public void run() {
                            lostCustomerNoteService.processLostCustomerNoteInfo(shop.getId(),current_time);
                        }
                    });
                }
            }
        } catch (Exception e) {
            logger.error("[提醒中心]流失客户提醒定时任务异常, 异常信息:", e);
        } finally {
