...
define(function(require,exports,module){
	...
	var formModule = require("./formData");
	var template = require("./../libs/artTemplate/artTemplate");
	var pageModule = require("./paging");
	...
	exports.fill = function(args){
		...
		form.off("click",".search_btn").on("click",".search_btn",function(){
            if(checkModule.check("#"+formid)){
            	if(method && method == "post"){
            		ajaxModule.post({
                        url : serviceUrl,
                        data : $.extend(formModule.get(formid),{page:1}),
                        success : success
                    });
            	}else{
            		ajaxModule.get({
			...
			dialogModule.confirm("确认删除？",function(){
				ajaxModule.post({
					...
					success:function(json){
						...
						if(!json.success){
							dialogModule.info(json.errorMsg || json.message,3);
						}else{
							dialogModule.info("删除成功",1,2,function(){
								ajaxModule.get({
									url:serviceUrl,
									data:$.extend({page:p},formModule.get(formid)),
									success:success
								});
							},true);
						}
		...
		var backFn = function(p){
            var top = $(".table_content").offset().top;
		...
		if(autoLoad && autoLoad=="true"){
			if(method && method == "post"){
				ajaxModule.post({
					url:serviceUrl,
					data:$.extend({page:pageNum},datas),
					success:success
				});
        	}else{
        		ajaxModule.get({
