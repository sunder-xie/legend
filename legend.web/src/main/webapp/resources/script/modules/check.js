...
define(function(require,exports,module){
	...
	var valid = function(obj,tipType){
		...
		var finalResult = true; //验证返回的最终结果
		if(!rule || rule == ""){
			//验证属性不符合规则跳出循环
			return true;
		}
		...
		$.each(regList, function(k,v){
			...
			if(validResult && validResult.result){
				...
				if(tipType == "tips"){
					...
					//弹框方式提示
					dialog.info(label+validResult.msg,3,2);
					obj.focus();
					// obj.css({
					// 	"box-shadow":"0 0 4px #f77c7e inset",
					// 	"border":"1px solid #f77c7e"
					// });
				}
				finalResult = false;
				return false;
	...
	function scrollToElement($e) {
		...
		if(scrollTop > range[1] || scrollTop < range[0]) {
			$(window).scrollTop(scrollTop - tipsHeight);
		}
	}
	...
	module.exports = {
		...
		check2 : function(scope){
			var result = true;
			$(elems,scope || "body").each(function(){
				$(this).trigger("blur");
				result = valid($(this),"alert");
				if(!result){
					return false;
				}
			});
