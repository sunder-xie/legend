...
define(function(require,exports,module){
	...
	var getHtml = function(obj,json){
		...
		if(hasTemplate.size()<1){
			...
			template = [
	            '<div class="qxy_input_downlist"'+(dataColsJson["width"]?'':'style="width:'+(dataColsLen*150)+'px"')+'>',
	            	'<ul class="list_title">',
			            //循环下拉列表头
			            '<% var index=0; %>',
			            '<li '+(dataColsJson["width"]?'style="width:'+widthCount+'px"':'')+'><% for(var name in dataColsJson){ %>',
			            '<% if(name == "width") {continue;} %>',
			            '<b style="width:',
			            //此处百分比减-1为了兼容ie7
			            '<%= (dataColsJson["width"] && dataColsJson["width"][index]+"px;") || ~~(100/dataColsLen) + "%;" %>">',
			            '<%= dataColsJson[name]%></b>',
			            '<% index++ %>',
			            '<%}%></li>',
		            '</ul>',
		            ...
		            '<ul class="list_content">',
			            ...
			            '<%for(var i=0;i<templateData.length;i++){',
				            ...
				            '%>',
				            '<% index=0; %>',
				            '<li><% for(var name in dataColsJson){ %>',
				            '<% if(name == "width") {continue;} %>',
							'<span style="width:',
							//此处百分比减-1为了兼容ie7
							'<%= (dataColsJson["width"] && dataColsJson["width"][index]+"px;") || ~~(100/dataColsLen) + "%;" %>">',
			...
			var html = temp({templateData:json,dataColsJson:dataColsJson,dataColsLen:dataColsLen});
			return html;
		}else{
			template = hasTemplate.html();
			var temp = tpl.compile(template);
			var html = temp({templateData:json});
			return html;
		}
	...
	$("body").on("click","input[service_url]",function(e){
		...
		var showName = $this.attr("name"); //输入框提交name(key)
		var showKey = $this.attr("show_key");//点击下拉列表每一行后，input内要填入该行的哪一个字段
		var hiddenName = $this.attr("hidden_name"); //输入框对应的隐藏id
		var hiddenKey = $this.attr("hidden_key"); //隐藏id对应json里的key
		var searchKey = $this.attr("search_key"); //要去对应json里搜索的关键字的key
		var remote = $this.attr("remote") || false; //是否在服务端进行模糊匹配
		var callbacks = $this.attr("callback_fn"); //单击li或回车li后的回调方法
		...
		var beforeSend  = function(){
			...
			//$this.addClass("qxy_input_ajax");
		}

		//根据配置的字段列表，去获取对应的值，并返回json.用作查询条件
		var getDatas = function(keys){
			var datas = {};
			var extDatas = $this.attr("ext_data"); //外部定义的查询条件方法
			var formItem = $this.parents(".form_item");

			// 此处是为了兼容ftl引用downlist模块，因为ftl没有form_item结构,这里限制在表格里
			if(formItem.size() == 0){
				formItem = $this.parents("tr");
			}
		...
		var success = function(json){
			...
			/*----兼容ie7 start----*/
			// if(low_ie){
			// 	$this.parentsUntil(".qxy_dialog").css({"z-index":15});
			// }
			/*----兼容ie7 end----*/
			$this.after(getHtml($this,json.data));

			$this.next(list).css({"min-width":$this.outerWidth()-2});
			//如果下拉列表无数据同时无新增按钮将不显示。
			var addBtn = $(".qxy_add_btn",list);
			if($(li,list).size()<=0 && addBtn.size()<1){
				colseList();
			}
		...
		var enterLi = function(obj){
			...
			var scope = $this.parents(".form_item");

			// 此处是为了兼容ftl引用downlist模块，因为ftl没有form_item结构,这里限制在表格里
			if(scope.size() == 0){
				scope = $this.parents("tr");
			}
		...
		$this.off("keyup").on("keyup",function(e){
			...
			switch(e.keyCode){
				case 38 :
					...
					$(".list_content",list).scrollTop(current*liHeight);//滚动条滚动
					break;
				case 40 :
					//键盘向下
	...
	var colseList = function(){
		/*----兼容ie7 start----*/
		// if(low_ie){
		// 	$(list).parentsUntil(".qxy_dialog").css({"z-index":"0"});
		// }
		/*----兼容ie7 end----*/
		$(list).remove();
	}
