//dict模块封装
define("legend/resources/online/script/region-debug", [ "./ajax-debug", "./dialog-debug", "../libs/layer/layer-debug" ], function(require, exports, module) {
    ...
    exports.init = function(scope) {
        ...
        var getData = function(obj, p_id, id, evt) {
            ...
            var success = function(json, id) {
                var p_id = 1;
                if (json.data[0] != null) p_id = json.data[0].id;
                $.each(json.data, function(k, v) {
                    if (id == v.id) {
                        html += "<option selected='selected' value='" + v.id + "'>" + v.regionName + "</option>";
                        p_id = id;
                    } else {
                        html += "<option value='" + v.id + "'>" + v.regionName + "</option>";
                    }
                });
                obj.html(html);
                ld(obj, p_id);
        ...
        var ld = function(obj, p_id, p_name, evt) {
            ...
            var index = selects.index(obj);
            if (selects.length == index + 1) return;
        ...
        if (input.val() != "") {
            ajaxModule.get({
                ...
                success: function(result) {
                    if (result.success) {
                        getData(firstSelect, result.data[0].parentId, input.val(), "change");
                    } else {
