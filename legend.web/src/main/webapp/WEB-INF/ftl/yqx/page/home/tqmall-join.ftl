<#include "yqx/tpl/home-header.ftl">
<link rel="stylesheet" href="${BASE_PATH}/static/css/page/home/tqmall-join.min.css?b2073ba3c1da75f7a98dbe3e5c30d38a">
<body>
<div class="wrap">
    <article class="page-panel slider-wrap tq-slider-wrap" id="sliderWrap">
        <section class="page page-1 tq-slider-page" data-id="淘汽知道">
                <div class="stage">
                    <article class="summary-box summary-box-1">
                        <section class="summary">
                            <h1 class="summary-title">淘汽知道</h1>
                            <p class="summary-section">修车有问题，找淘汽知道<br/>一键提问，专家免费回答</p><br/>
                            <p class="summary-section">几百位专家实时在线为您解答疑惑<br/>提供海量维修资料、维修案例</p><br/>
                            <ul class="digit-box">
                                <li class="q">
                                    <img src="${BASE_PATH}/static/img/page/home/tqmall-join/01_q.jpg">
                                    <p class="digit-cn">问题数</p>
                                    <p class="q-digit">30,242+</p>
                                </li>
                                <li class="a">
                                    <img src="${BASE_PATH}/static/img/page/home/tqmall-join/01_a.jpg">
                                    <p class="digit-cn">有效回答数</p>
                                    <p class="a-digit">238,196+</p>
                                </li>
                                <li class="t">
                                    <img src="${BASE_PATH}/static/img/page/home/tqmall-join/01_t.jpg">
                                    <p class="digit-cn">平均回答时间</p>
                                    <p class="t-digit">547"</p>
                                </li>
                            </ul>
                        </section>
                    </article>
                    <div class="main-stage main-stage-1">
                        <img class="chunk-1" src="${BASE_PATH}/static/img/page/home/tqmall-join/01_up.png" alt="手机上">
                        <img class="chunk-2" src="${BASE_PATH}/static/img/page/home/tqmall-join/01_down.png" alt="手机上">
                    </div>
                </div>
            </section>
        <section class="page page-2 tq-slider-page" data-id="客户营销">
                <div class="stage">
                    <article class="summary-box summary-box-2">
                        <section class="summary">
                            <h1 class="summary-title">客户营销</h1>
                            <p class="summary-section">为您提供独一无二的客户分析、客情维护、营销中心，<br/>唤醒沉睡的客户资料 </p>
                            <ul class="summary-section">
                                <li class="decimal">实时掌握门店客户情况，多少新客户，多少老客户一清二楚</li>
                                <li class="decimal">及时获取客户需求，多少客户保险到期，多少客户保养到期清清楚楚；更有微信、短信、App消息多种方式触达用户。</li>
                                <li class="decimal">精确筛选客户，推送定制化、个性化的营销套餐</li>
                            </ul>
                        </section>
                    </article>
                    <div class="main-stage main-stage-2">
                        <div class="chunk-box chunk-box-1">
                            <img class="chunk chunk-1" src="${BASE_PATH}/static/img/page/home/tqmall-join/02_khyx.png" alt="客户营销">
                            <div class="fragment-box fragment-box-1-1">
                                <img class="fragment" src="${BASE_PATH}/static/img/page/home/tqmall-join/02_khfx.png" alt="客户分析">
                            </div>
                            <div class="label label-up"><span counter="1">客户分析</span></div>
                        </div>
                        <div class="chunk-box chunk-box-2">
                            <img class="chunk chunk-2" src="${BASE_PATH}/static/img/page/home/tqmall-join/02_yxzx.png" alt="营销中心">
                            <div class="label label-up"><span counter="2">营销中心</span></div>
                        </div>
                        <div class="chunk-box chunk-box-3">
                            <img class="chunk chunk-3" src="${BASE_PATH}/static/img/page/home/tqmall-join/02_kqwh.png" alt="客情维护">
                            <div class="fragment-box fragment-box-3-1">
                                <img class="fragment" src="${BASE_PATH}/static/img/page/home/tqmall-join/02_kqwh_s.png" alt="客情维护">
                            </div>
                            <div class="label label-down"><span counter="3">客情维护</span></div>
                        </div>
                    </div>
                </div>
        </section>
        <section class="page page-3 tq-slider-page" data-id="引流活动">
                <div class="stage">
                    <article class="summary-box summary-box-3">
                        <section class="summary">
                            <h1 class="summary-title">引流活动</h1>
                            <p class="summary-section">
                                天猫商城<br/>
                                平安保险多渠道<br/>
                                多类型活动<br/>
                                吸引用户到店消费
                            </p>
                        </section>
                    </article>
                    <div class="main-stage main-stage-3">
                        <div class="chunk-box-1">
                            <img class="chunk-1" src="${BASE_PATH}/static/img/page/home/tqmall-join/03_zxyl.png" alt="在线引流">
                            <div class="layer layer-1"></div>
                            <div class="layer layer-2"></div>
                            <div class="fragment-box fragment-box-3-1">
                                <img class="fragment fragment-3-1" src="${BASE_PATH}/static/img/page/home/tqmall-join/03_tmsc.png" alt="天猫商城">
                            </div>
                            <div class="fragment-box fragment-box-3-2">
                                <img class="fragment fragment-3-2" src="${BASE_PATH}/static/img/page/home/tqmall-join/03_pabxdqd.png" alt="平安保险多渠道">
                            </div>
                            <div class="fragment-box fragment-box-3-3">
                                <img class="fragment fragment-3-3" src="${BASE_PATH}/static/img/page/home/tqmall-join/03_dlxhd.png" alt="多类型活动">
                            </div>
                        </div>
                    </div>
                </div>
            </section>
    </article>
    <aside class="slider-control tq-slider-control">
        <ul>
            <li class="active" content="淘汽知道"></li>
            <li content="客户营销"></li>
            <li content="引流活动"></li>
        </ul>
    </aside>
    <footer class="down tq-slider-down">
        <p>升级为淘汽云修，即可享受云修生态圈全应用</p>
        <p>加盟热线：<strong>400-9937-288</strong></p>
        <div class="down-arrows tq-slider-arrows"></div>
    </footer>
</div>
<script src="${BASE_PATH}/static/js/page/home/tqmall-join.js?4844ec4a7fff82da9bb524668dd5853a"></script>
</body>
</html>