...
<#else>
    ...
    <article>
        <ul class="box order-info">
            <li id="开单时间" class="time"><i class="key">开单时间：</i><i class="text">${orderInfo.createTime?string("yyyy-MM-dd HH:mm")}</i></li>
    ...
    <#if debitBillFlowList?exists || orderInfo.signAmount gt 0>
        ...
        <ul class="box footer-box">
            <li class="fl" id="打印时间">
                <i class="five">打印时间：</i><i class="print-time text"><#if orderInfo.printTime>${orderInfo.printTime?string("yyyy-MM-dd HH:mm")}<#else>${.now?string("yyyy-MM-dd HH:mm")}</#if></i>
            </li>
            <li class="fr">
