    ...
    <div class="pwdconfirm_part dialog_height_200" data-tpl-ref="new-part-tpl">
        ...
        <div class="bottom">
            ...
            <span id="resetExportPWD" class="reset">
                <a href="${BASE_PATH}/shop/conf/exportpassword/reset/ng">重置密码</a>
            </span>
        </div>
    </div>
</script>
...
<script>
    ...
    (function ($) {
        ...
        exportSecurity.tip = function (option) {
            ...
            // 密码提示设置 模板
            var set_template = template('exportpwd_set_template', {'title': title});
            $.when(
                    $.ajax({
            ...
            doc.on("click", ".set_submit", function () {
                window.document.location.href = BASE_PATH + "/shop/conf/exportpassword/ng";
            }).on("click", ".set_cancel", function () {
                dialogIndex && layer.close(dialogIndex);
            });
        };
        ...
        exportSecurity.confirm = function (option) {
            ...
            doc.on("click", args.dom, function () {
                ...
                // 密码输入框 模板
                var confirm_template = template('exportpwd_confirm_template', {});
                $.when(
                        $.ajax({
                            url: BASE_PATH + "/shop/conf/exportpassword/precheck",
                            data: ""
        ...
        exportSecurity.tipAndConfirm = function (option) {
            ...
            doc.on("click", args.dom, function () {
                ...
                // 密码设置框
                var set_template = template('exportpwd_set_template', {'title': exportSecurity.exportBrief});
                // 密码输入框
                var confirm_template = template('exportpwd_confirm_template', {blank: blank});
            ...
            $.ajax({
                ...
                if (json.success) {
                    if (_current.type == 'export_excel') {
                        dialog.success("密码验证通过,正在导出Excel...");
                        _current.args["callback"] && _current.args["callback"].call(_exportEle);
                    } else {
                        ret = _current.args["callback"] && _current.args["callback"].call(_exportEle);
                    }
