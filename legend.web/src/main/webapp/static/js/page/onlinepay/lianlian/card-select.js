...
$(function () {
    ...
    var Model = {
        ...
        getSupportedCardList: function () {
            var url = BASE_PATH + '/onlinepay/lianlian/support-debit-card-list';
            return $.get(url);
        },
        // 获取支持的信用卡列表
        getSupportedCreditList: function () {
            var url = BASE_PATH + '/onlinepay/lianlian/support-credit-card-list';
            return $.get(url);
        },
        // 获取过去已支付过的银行卡列表
        getPayedCardList: function () {
            var ucShopId = $('[name="ucShopId"]').val();
            var url = BASE_PATH + '/onlinepay/lianlian/card-list?ucShopId=' + ucShopId;
            return $.get(url);
        },
        // 匹配银行卡信息是否一致
        matchedCardInfo: function (params) {
            var url = BASE_PATH + '/onlinepay/lianlian/check-card';
            return $.ajax({
                url: url,
                data: params,
                global: false
        ...
        toPay: function (params) {
            var url = BASE_PATH + '/onlinepay/pay-choice';
            return $.ajax({
                type: 'POST',
                url: url,
    ...
    // 银行卡类型选择
        ...
        .on('blur', '.js-card-no-input', function () {
            ...
            data['bankCode'] = $selectedCard[0].dataset['bankCode'];
