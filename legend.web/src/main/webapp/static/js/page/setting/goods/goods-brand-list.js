...
$(function(){
    ...
    seajs.use(['table', 'dialog', 'check', 'formData'], function (tb, dg, ck, fd) {
        ...
        tb.init({
            //表格数据url，必需
            url: BASE_PATH + '/shop/goods_brand/search',
            //表格数据目标填充id，必需
            fillid: 'tableListCon',
            //分页容器id，必需
            pageid: 'pagingCon',
        ...
        doc.on('click', '.js-edit', function () {
            ...
            thisInput.show();
            thisText.hide();
            thisInput.find('input[name="brandName"]').val(thisText.text().trim());
            thisInput.find('input[name="brandName"]').focus();
        });
        ...
        function saveTypeName($this){
            ...
            //.提交到后台
            var goodsbrand = fd.get($this.parents('tr'));
            doSave(goodsbrand, function () {
                //.样式切换
                var thisInput = $this.parents('tr').find('.form-item');
                var thisText = $this.parents('tr').find('.form-label');
                var name = $this.val();
        ...
        doc.on('click', '.js-delete', function () {
            ...
            dg.confirm('确定要删除吗？', function () {
                $.ajax({
                    type: 'post',
                    url: BASE_PATH + '/shop/goods_brand/goods-brand-delete',
                    data: {id: id},
        ...
        doc.on('click', '.js-confirm', function () {
            ...
            var goodsbrand = fd.get('.js-goodsbrand-form');
            doSave(goodsbrand, function () {
                window.location.href = BASE_PATH + '/shop/goods_brand/goods-brand-list';
            });
        });
